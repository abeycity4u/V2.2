...
async function confirmAction(message, options = {}) {
    if (window.AppConfirm && typeof window.AppConfirm.ask === 'function') {
        return await window.AppConfirm.ask(message || 'Are you sure you want to proceed?', options);
    }
    return true;
}
...
async function deleteRecord(recordId, recordType) {
    if (!await confirmAction('Delete this record? This action cannot be undone.', {title:'Delete daily record?', confirmText:'Delete'})) return;
