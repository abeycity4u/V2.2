...
$dailyFeedItemsStmt = $pdo->prepare("SELECT id, item_name, unit, current_stock, unit_cost FROM stock_items WHERE farm_id = ? AND is_active = 1 AND feed_category = 'layer' AND farm_type IN ('poultry', 'both') ORDER BY item_name ASC");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['save_record'])) {
        ...
        if (!$dateObject || $dateObject->format('Y-m-d') !== $recordDate) {
            ...
            header('Location: layers_daily_record.php?month=' . urlencode($yearMonth) . '&cycle_id=' . (int)$selectedCycleId);
            exit();
        }

        $cycleIdForSave = ($cycleEnabled && $selectedCycleId > 0) ? $selectedCycleId : null;
        if ($cycleIdForSave !== null) {
            $cycleCheck = $pdo->prepare("SELECT id, start_date, expected_end_date FROM production_cycles WHERE id = ? AND farm_id = ? AND farm_type = 'poultry' AND production_type = 'layer' AND status = 'active' LIMIT 1");
            ...
            if (!$cycle || $recordDate < $cycle['start_date'] || (!empty($cycle['expected_end_date']) && $recordDate > $cycle['expected_end_date'])) {
                $_SESSION['error'] = 'The selected layer cycle is invalid for this record date.';
                header('Location: layers_daily_record.php?month=' . urlencode($yearMonth) . '&cycle_id=' . (int)$selectedCycleId);
                exit();
            }
        }
        ...
        try {
            ...
            $eggProduction = (int)$nonNegative($_POST['egg_production'] ?? 0, 'Egg production');
        ...
        if ($eggProduction > $openingStock) {
            $_SESSION['error'] = 'Egg production cannot exceed the opening flock for a single day.';
            header('Location: layers_daily_record.php?month=' . urlencode($yearMonth) . '&cycle_id=' . (int)$selectedCycleId);
            exit();
        }
        $calculatedLayingRate = round(($eggProduction / $openingStock) * 100, 2);
        $calculatedCrates = round($eggProduction / 30, 2);

        // Cycle continuity: the next day's opening flock must equal the previous day's closing flock.
        if ($cycleIdForSave !== null) {
            $previousStmt = $pdo->prepare("SELECT record_date, opening_stock, mortality FROM layer_daily_records WHERE farm_id = ? AND cycle_id = ? AND record_date < ? ORDER BY record_date DESC LIMIT 1");
            ...
            $nextStmt = $pdo->prepare("SELECT record_date, opening_stock, mortality FROM layer_daily_records WHERE farm_id = ? AND cycle_id = ? AND record_date > ? ORDER BY record_date ASC LIMIT 1");
...
<div class="col-md-4 mb-3">
 ...
 <script src="<?php echo BASE_URL; ?>/assets/js/edit-modal.js"></script>
    ...
    <?php if ($canDelete): ?>
    // Delete record
    function deleteLayerDailyRecord(recordId) {
