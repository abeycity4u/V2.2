...
$cycleStmt = $pdo->prepare("SELECT id, cycle_code, production_type, start_date FROM production_cycles WHERE farm_id = ? AND status = 'active' AND farm_type = 'poultry' AND production_type = 'layer' ORDER BY start_date DESC, id DESC");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_transaction'])) {
    ...
    try {
        ...
        create_manual_feed_transaction($pdo, $tenantFarmId, $itemId, $type, $quantity, $date,
            ...
            'poultry', 'layer', (int)$_SESSION['user_id']);
        $pdo->commit();
                                    ...
                                    <tr class="<?php echo (!empty($trans['is_reversed']) || !empty($trans['reversal_of_id'])) ? 'feed-audit-row' : 'feed-operational-row'; ?>">
                                        ...
                                        <td>
                                            ...
                                            <?php if (!empty($trans['is_reversed'])): ?>
                                                <span class="badge bg-secondary ms-1" title="This original Daily Record movement was reversed by a linked restoration transaction.">Reversed</span>
                                            <?php elseif (!empty($trans['reversal_of_id'])): ?>
                                                <span class="badge bg-info text-dark ms-1" title="This movement reverses an earlier Daily Record transaction.">Restoration</span>
                                            <?php endif; ?>
                                        </td>
                                        <td class="fw-bold <?php echo $trans['transaction_type'] === 'received' ? 'text-success' : 'text-danger'; ?>">
                                            <?php echo $trans['transaction_type'] === 'received' ? '⬆ +' : '⬇ -'; ?><?php echo number_format((float)$trans['quantity'], 2); ?>
                                        </td>
