...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($categoryId <= 0) {
        ...
        try {
            ...
            if (!empty($items)) {
                throw new RuntimeException('This category is still assigned to ' . count($items) . ' inventory item' . (count($items) === 1 ? '' : 's') . '. Move or permanently remove those items before deleting the category.');
            }
