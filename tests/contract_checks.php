...
assertContains('stock_apply_movement(', readFileOrFail($root . '/api/update_stock.php'), 'Stock updates must use the canonical stock ledger service.');
...
foreach (['UPDATE stock_items SET is_active = 0 WHERE id = ? AND farm_id = ?', 'UPDATE stock_items SET is_active = 1 WHERE id = ? AND farm_id = ?'] as $tenantMutation) {
    assertContains($tenantMutation, $inventory, 'Inventory destructive mutations must include the active farm.');
}
assertNotContains('DELETE FROM stock_transactions WHERE stock_item_id = ? AND farm_id = ?', $inventory, 'Inventory must never purge stock ledger history.');
