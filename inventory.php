...
// roles may receive read access without pretending to be a livestock manager.
...
if ($isOwnerOrAdmin) {
    ...
    $enabledTypes = enabledFarmTypes();
    $farmType = count($enabledTypes) > 1 ? 'all' : ($enabledTypes[0] ?? 'all');
}
...
$inactiveQuery = "SELECT si.*, ic.category_name,
                         (SELECT COUNT(*) FROM stock_transactions st WHERE st.stock_item_id = si.id AND st.farm_id = si.farm_id) AS transaction_count,
                         (SELECT COUNT(*) FROM stock_transactions st
                          WHERE st.stock_item_id = si.id AND st.farm_id = si.farm_id
                            AND st.transaction_type = 'received'
                            AND st.remarks = 'Initial stock entry'
                            AND COALESCE(st.is_reversed, 0) = 0
                            AND COALESCE(st.reversal_of_id, 0) = 0) AS setup_transaction_count
                  FROM stock_items si
                  JOIN inventory_categories ic ON si.category_id = ic.id AND ic.farm_id = si.farm_id
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['delete_category'])) {
        ...
        try {
            ...
            if (!empty($items)) {
                $count = count($items);
                throw new RuntimeException('This category is still assigned to ' . $count . ' inventory item' . ($count === 1 ? '' : 's') . '. Move or permanently remove those items before deleting the category.');
            }
            ...
            if ($pdo->inTransaction()) $pdo->rollBack();
            $_SESSION['error'] = safeUserExceptionMessage($e, 'The inventory category could not be deleted.');
        }
    ...
    if (isset($_POST['add_item'])) {
        ...
        try {
            ...
            $stmt = $pdo->prepare("INSERT INTO stock_items
                ...
                VALUES (?, ?, ?, 0, ?, ?, ?, ?, 0)");
            $stmt->execute([$currentFarmId, $itemName, $categoryId, $minStock, $unit, $farmType, $feedCategory]);
            $itemId = (int)$pdo->lastInsertId();

            if ($initialStock > 0) {
                stock_apply_movement($pdo, $currentFarmId, $itemId, 'received', $initialStock, $receivedDate,
                    'Initial stock entry', (int)($_SESSION['user_id'] ?? 0), $farmType, $feedCategory,
                    null, 'inventory_manual', null, $unitCost);
            }
            $pdo->commit();
    ...
    if (isset($_POST['update_stock'])) {
        ...
        $type = trim((string)($_POST['transaction_type'] ?? ''));
        $quantity = (float)str_replace(',', '', (string)($_POST['quantity'] ?? 0));
        $incomingUnitCostRaw = isset($_POST['unit_cost']) ? trim((string)$_POST['unit_cost']) : '';
        $incomingUnitCost = $incomingUnitCostRaw !== ''
            ? (float)str_replace(',', '', $incomingUnitCostRaw)
            : null;

        try {
            if ($itemId <= 0 || !in_array($type, ['received', 'used'], true) || $quantity <= 0) {
                throw new RuntimeException('Please select an item, transaction type, and a quantity greater than zero.');
            }
            if ($type === 'received' && $incomingUnitCost === null) {
                throw new RuntimeException('Enter the actual unit cost for received stock so historical profitability remains accurate when prices change.');
            }
            if ($type === 'received' && $incomingUnitCost < 0) {
            ...
            if ($pdo->inTransaction()) $pdo->rollBack();
            $_SESSION['error'] = safeUserExceptionMessage($e, 'The inventory stock could not be updated.');
        }
        header('Location: inventory.php');
        exit();
    }
    ...
    if (isset($_POST['delete_item'])) {
        ...
        $itemId = (int)($_POST['item_id'] ?? 0);
        try {
            if ($itemId <= 0) throw new RuntimeException('Please select a valid inventory item.');
            $pdo->beginTransaction();

            $historyStmt = $pdo->prepare("SELECT id, transaction_type, remarks, COALESCE(is_reversed,0) AS is_reversed, COALESCE(reversal_of_id,0) AS reversal_of_id FROM stock_transactions WHERE stock_item_id = ? AND farm_id = ? ORDER BY id");
            $historyStmt->execute([$itemId, $currentFarmId]);
            $history = $historyStmt->fetchAll(PDO::FETCH_ASSOC);
            $setupOnly = count($history) === 1
                && ($history[0]['transaction_type'] ?? '') === 'received'
                && ($history[0]['remarks'] ?? '') === 'Initial stock entry'
                && (int)($history[0]['is_reversed'] ?? 0) === 0
                && (int)($history[0]['reversal_of_id'] ?? 0) === 0;

            if (empty($history) || $setupOnly) {
                if ($setupOnly) {
                    $pdo->prepare("DELETE FROM stock_transactions WHERE stock_item_id = ? AND farm_id = ?")->execute([$itemId, $currentFarmId]);
                }
                $pdo->prepare("DELETE FROM stock_items WHERE id = ? AND farm_id = ?")->execute([$itemId, $currentFarmId]);
                $pdo->commit();
                $_SESSION['success'] = 'Inventory item deleted successfully.';
            } else {
                $pdo->prepare("UPDATE stock_items SET is_active = 0 WHERE id = ? AND farm_id = ?")->execute([$itemId, $currentFarmId]);
                $pdo->commit();
                $_SESSION['success'] = 'Item archived successfully. Operational stock history was preserved.';
            }
        } catch (Throwable $e) {
            if ($pdo->inTransaction()) $pdo->rollBack();
            $_SESSION['error'] = safeUserExceptionMessage($e, 'The inventory item could not be deleted or archived.');
        }
        header('Location: inventory.php');
        exit();
    }
    ...
    if (isset($_POST['purge_item'])) {
        ...
        try {
            ...
            if (!empty($history) && !$setupOnly) {
                throw new RuntimeException('This item has operational stock history, so it must remain archived to preserve the audit trail. You can restore it at any time.');
            }
            if ($setupOnly) {
                $pdo->prepare("DELETE FROM stock_transactions WHERE stock_item_id = ? AND farm_id = ?")->execute([$itemId, $currentFarmId]);
            }
            $pdo->prepare("DELETE FROM stock_items WHERE id = ? AND farm_id = ?")->execute([$itemId, $currentFarmId]);
            $pdo->commit();
            $_SESSION['success'] = "Inventory item deleted permanently.";
        } catch (RuntimeException $e) {
            if ($pdo->inTransaction()) $pdo->rollBack();
            $_SESSION['warning'] = $e->getMessage();
        } catch (Throwable $e) {
            if ($pdo->inTransaction()) $pdo->rollBack();
            $_SESSION['error'] = safeUserExceptionMessage($e, 'The inventory item could not be permanently deleted.');
        }
    ...
    <div class="container-fluid mt-4">
        <div class="inventory-shell">
            ...
            <?php if ($canManageInventory && !empty($inactiveItems)): ?>
                <div class="d-flex justify-content-end mb-3"><button class="btn btn-outline-secondary btn-sm" type="button" data-bs-toggle="modal" data-bs-target="#archivedItemsModal"><i class="bi bi-archive me-1"></i> Archived Items <span class="badge text-bg-secondary ms-1"><?php echo count($inactiveItems); ?></span></button></div>
                <div class="modal fade" id="archivedItemsModal" tabindex="-1" aria-labelledby="archivedItemsTitle" aria-hidden="true"><div class="modal-dialog modal-xl modal-dialog-scrollable"><div class="modal-content"><div class="modal-header"><div><h5 class="modal-title" id="archivedItemsTitle"><i class="bi bi-archive me-2"></i>Archived Inventory Items</h5><div class="small text-muted">Archived items stay in historical reports but cannot be selected for new stock or feed activity.</div></div><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div><div class="modal-body p-0"><div class="table-responsive"><table class="table table-hover align-middle mb-0"><thead class="table-dark"><tr><th>Item</th><th>Category</th><th>Stock</th><th>Unit</th><th>History</th><th>Actions</th></tr></thead><tbody>
                <?php foreach ($inactiveItems as $inactive): ?><tr><td class="fw-semibold"><?php echo htmlspecialchars($inactive['item_name']); ?></td><td><?php echo htmlspecialchars($inactive['category_name']); ?></td><td><?php echo number_format((float)$inactive['current_stock'],2); ?></td><td><?php echo htmlspecialchars($inactive['unit']); ?></td><td><?php echo (int)$inactive['transaction_count']; ?> transaction(s)</td><td><div class="d-flex gap-2 flex-wrap"><form method="POST" class="mb-0"><input type="hidden" name="item_id" value="<?php echo $inactive['id']; ?>"><button type="submit" name="restore_item" class="btn btn-sm btn-outline-success"><i class="bi bi-arrow-counterclockwise"></i> Restore</button></form><?php $setupOnlyHistory=(int)($inactive['transaction_count']??0)===1&&(int)($inactive['setup_transaction_count']??0)===1; if((int)($inactive['transaction_count']??0)===0||$setupOnlyHistory): ?><form method="POST" class="mb-0" data-confirm="This item has no operational history. Its setup/opening entry, if present, will also be removed. Continue?" data-confirm-title="Delete inventory item permanently?" data-confirm-button="Delete permanently"><input type="hidden" name="item_id" value="<?php echo $inactive['id']; ?>"><button type="submit" name="purge_item" class="btn btn-sm btn-outline-danger"><i class="bi bi-trash"></i> Delete permanently</button></form><?php else: ?><span class="badge bg-secondary align-self-center"><i class="bi bi-shield-lock"></i> Protected history</span><?php endif; ?></div></td></tr><?php endforeach; ?>
                </tbody></table></div></div><div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button></div></div></div></div>
            <?php endif; ?>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <?php if ((int)$category['item_count'] > 0): ?>
                                                        <button type="button" class="btn btn-sm btn-outline-secondary" disabled title="This category is still assigned to inventory items.">
                                                            <i class="bi bi-shield-lock"></i> In use
                                                        </button>
                                                    <?php else: ?>
                                                        <form method="POST" data-confirm="Delete this empty inventory category? This action cannot be undone." data-confirm-title="Delete inventory category?" data-confirm-button="Delete category" class="mb-0">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-12 mb-3">
                                    ...
                                    <small class="text-muted">Classifies how the item is used. Choose General / Non-feed item for vaccines, medicines, tools and other stock that is not feed.</small>
                                </div>
                            </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="date" name="received_date" class="form-control" value="<?php echo date('Y-m-d'); ?>" max="<?php echo date('Y-m-d'); ?>" required>
                                    <small class="text-muted">Date the opening stock was received.</small>
                                </div>
                                <div class="col-md-6 mb-3">
                    ...
                    <form method="POST" id="updateStockForm">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="mb-3" id="unitCostWrapper">
                                ...
                                <input type="number" name="unit_cost" class="form-control" step="0.01" min="0" placeholder="Enter actual receipt price">
                                <small class="text-muted">Required for receipts. The actual price is snapshotted and blended into weighted-average inventory cost.</small>
                            </div>
